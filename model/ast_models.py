...
class ASTModel(nn.Module):
    def __init__(self, label_dim=527,
                 ...
                 pretrain_stage=True, load_pretrained_mdl_path=None, xvector_d=192):
...
################################################################################################################
            # mlp head for fine-tuning ：768—>768; 768——>1
            self.mlp_head = nn.Sequential(nn.LayerNorm(self.original_embedding_dim),
                                          nn.Linear(self.original_embedding_dim, 2*self.original_embedding_dim),
                                          nn.Dropout(0.5),
                                          nn.GELU(),
                                          nn.Linear(2*self.original_embedding_dim, label_dim))
            # head2
            ...
            #speaker embedding: x-vector(192d) ——> spk_emb(768:4×192)
            self.speaker_embedding = nn.Sequential(nn.LayerNorm(xvector_d),
                                     nn.Linear(xvector_d, self.original_embedding_dim)              
                                     # nn.TransformerEncoder(self.encoder, num_layers=1)
...
################################################################################################################
    ...
    def finetuningcls(self, x):
        ...
        if self.cls_token_num == 2:
            ...
            x = x[:, 0]
        x = self.mlp_head(x)
        return 12*torch.tanh(x) #proj to -[12, 12]
    
    
###########################################################################################    
    def finetuningcls2(self, x):
        B = x.shape[0]
        x = self.v.patch_embed(x)
        if self.cls_token_num == 2:
            cls_tokens = self.v.cls_token.expand(B, -1, -1)
        ...
        if self.cls_token_num == 2:
            ...
            x = x[:, 0]
        freq_factor = self.mlp_head(x)
        time_factor = self.mlp_head2(x)
        return 12*torch.tanh(freq_factor), 2*torch.sigmoid(time_factor)
    
    
    def finetuningcls_spk(self, x, x_vector):
        B = x.shape[0]
        x = self.v.patch_embed(x)
        x_vector = self.speaker_embedding(x_vector)#.squeeze(1)
        ...
        feature_branch = x + x_vector 
        freq_factor = self.mlp_head(feature_branch)
        time_factor = self.mlp_head2(feature_branch)
        return 12*torch.tanh(freq_factor), 2*torch.sigmoid(time_factor)
    
    
    def finetuningemb(self, x):
        B = x.shape[0]
        x = self.v.patch_embed(x)
        if self.cls_token_num == 2:
            cls_tokens = self.v.cls_token.expand(B, -1, -1)
...
###########################################################################################
    ...
    def forward(self, x, task, cluster=True, mask_patch=400, x_vector=None):
        ...
        # custom task
            ...
            return self.finetuningcls2(x)
        elif task =='ft_spk':
            assert x_vector!=None
            return self.finetuningcls_spk(x, x_vector=x_vector)
        elif task =='ft_emb':
            return self.finetuningemb(x)
        ##############################################################################
        else:
